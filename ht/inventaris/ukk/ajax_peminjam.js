...
var xmlhttp;
function tambah(a){
	var url="../admin/peminjam_tambah.php?rand="+Math.random();
	var post="";
	ajax(url,post,"tambah");
	}
function tampil(){
	var url="../admin/peminjam_tampil.php?rand="+Math.random();
	var post="";
	ajax(url,post,"tampil");
	}
function simpan_peminjam(){
var kode=document.getElementById("kd_inventaris").value;
var karyawan=document.getElementById("kd_kar").value;
var petugas=document.getElementById("petugas").value;

if(petugas==""){
	alert('masukkan petugas!!!');
	document.getElementById("petugas").focus();
	return false;
	}
else{
	var url='../admin/peminjam_simpan.php';
	var post="kode="+kode+"&karyawan="+karyawan+"&petugas="+petugas;
	ajax(url,post,"tampil");
	}
	}
...
if(label==""){
	...
	var url='peminjam_proses.php';
	var post="kode="+kode+"&label="+label;
	ajax(url,post,"tampil");
	}
	}
